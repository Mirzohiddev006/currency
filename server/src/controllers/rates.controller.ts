...
export async function manualRefreshHandler(req: Request, res: Response) {
  ...
  res.json({
    success: true,
    message: "Yangilash boshlandi. Bu bir necha daqiqa olishi mumkin.",
  });
...
export async function manualBankRefreshHandler(req: Request, res: Response) {
  ...
  res.json({
    success: true,
    message:
      "Bank kurslarini yangilash boshlandi. Bu bir necha daqiqa olishi mumkin.",
  });

  ratesService
    .refreshBankRates(bankCode)
    .catch((err) => console.error("Background bank refresh error:", err));
}
