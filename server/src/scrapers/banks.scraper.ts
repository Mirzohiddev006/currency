...
function normalizeByNominal(value: number | null, nominal: number): number | null {
  if (value === null) {
    return null;
  }
...
function mergeRates(primary: BankRateResult[], fallback: BankRateResult[]): BankRateResult[] {
...
function parseDavrbankTable(html: string): BankRateResult[] {
  ...
  $('table tr').each((_, row) => {
    ...
    const currencyLabel =
      normalizeWhitespace($(cells[0]).find('img').attr('alt')) || normalizeWhitespace($(cells[0]).text());
...
function parseApexTable(html: string): BankRateResult[] {
  ...
  $('table tr').each((_, row) => {
    ...
    if (buyRate || sellRate) {
      rates.push({
        bankCode: 'apexbank',
        currency: normalizeWhitespace($(cells[0]).text()).replace(code, '').trim() || code,
        code,
        buyRate,
        sellRate,
      });
...
function parseBankUzDetail(html: string, bankCode: string): ScrapedBankPayload {
  ...
  $('.table-kurs .row').slice(1).each((_, row) => {
    ...
    const buyRate = normalizeByNominal(parseRate($(cells[3]).text()), nominal);
    const sellRate = normalizeByNominal(parseRate($(cells[4]).text()), nominal);
...
async function fetchKursUzRates(): Promise<BankRateResult[]> {
  if (kursUzCache && Date.now() - kursUzCache.fetchedAt < KURS_UZ_CACHE_TTL_MS) {
    ...
    kursUzPromise = (async () => {
      const responses = await Promise.all(
        KURS_UZ_SUPPORTED_CURRENCIES.map(async (currency) => {
          ...
          const $ = cheerio.load(`<table><tbody>${response.data}</tbody></table>`);
          ...
          $('tr').each((_, row) => {
            ...
            const slugFromValue =
              ...
              normalizeWhitespace($(cells[0]).text()).toLowerCase().replace(/\s+/g, '');
            const bankCode =
...
async function getKursUzRatesForBank(bankCode: string): Promise<BankRateResult[]> {
...
async function scrapeBankUzBank(bankCode: string, slug: string): Promise<ScrapedBankPayload> {
  return scrapeWithFallback(bankCode, async () => {
    const { data } = await scraperHttpClient.get(`${BANK_UZ_BASE_URL}/uz/currency/bank/${slug}`);
...
async function scrapeTbcBank(): Promise<ScrapedBankPayload> {
  return scrapeWithFallback('tbcbank', async () => {
    const { data } = await scraperHttpClient.get('https://tbcbank.uz/uz/currency/');
    return {
...
async function scrapeDavrbank(): Promise<ScrapedBankPayload> {
  return scrapeWithFallback('davrbank', async () => {
    const { data } = await scraperHttpClient.get('https://davrbank.uz/uz/exchange-rate');
    return {
...
async function scrapeApexBank(): Promise<ScrapedBankPayload> {
  return scrapeWithFallback('apexbank', async () => {
    const { data } = await scraperHttpClient.get('https://www.apexbank.uz/about/exchange-rates/', {
...
async function scrapeKursOnlyBank(bankCode: string): Promise<ScrapedBankPayload> {
  return {
...
export const BANK_SCRAPERS: Record<string, () => Promise<ScrapedBankPayload>> = {
  ...Object.fromEntries(
    BANK_UZ_BANKS.map((bank) => [bank.code, () => scrapeBankUzBank(bank.code, bank.bankUzSlug)])
  ),
